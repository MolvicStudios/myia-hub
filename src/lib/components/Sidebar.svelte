<script lang="ts">
	...
	function confirmDelete(id: string) {
		if (confirm($i18n('sidebar.deleteConfirm'))) {
			deleteChat(id);
		}
	}
...
>
	...
	<div class="flex items-center justify-between p-3 border-b border-slate-800">
		...
		<div class="flex items-center gap-1">
			...
			<button
				...
				title={$i18n('sidebar.closeSidebar')}
			>
				<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
	...
	<!-- Chat list -->
	<div class="flex-1 overflow-y-auto py-2" role="list" aria-label={$i18n('sidebar.chatList')}>
		{#each $chats as chat (chat.id)}
			...
			>
				...
				<div class="hidden group-hover:flex group-focus-within:flex items-center gap-0.5 pr-2">
					...
					<button
						type="button"
						aria-label="{$i18n('sidebar.delete')} {chat.title}"
						class="p-1 text-slate-500 hover:text-red-400 transition-colors"
						onclick={() => confirmDelete(chat.id)}
						title={$i18n('sidebar.delete')}
					>
						<svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
